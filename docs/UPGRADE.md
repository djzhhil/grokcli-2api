...
## 场景 B：已是 hybrid，仅升级应用版本（含 1.x → 2.0.2）

适用于：已在用 PostgreSQL + Redis，从 **1.9.x / 2.0.0 / 2.0.1** 升到 **2.0.2**。
...
# compose 中 image 改为 :2.0.2 或 :latest 后：
docker compose up -d
...
| 额度 | 2.0.2 起类型/用量持久化；历史 error 壳不当有效额度 |
